...
vu.sop.load = async function(basePath) {
    try {
        ...
        let tfJsLoad =  vu.sop.loadJs(basePath + '/js/libs/tensorflowjs/3.9.0/tf.min.js');
        await tfJsLoad;

        let tfJsWasmLoad =  vu.sop.loadJs(basePath + '/js/libs/tensorflowjs/3.9.0/tf-backend-wasm.min.js');
    ...
    try {
        if (!vu.sop.ui.isDeviceCompatible()) {
            await vu.error.showError(new vu.error.LoadError('deviceNotSupported'));    
        }
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = function(evt){
        ...
        img.onload = function() {
            ...
            if (maxSize > img.height && maxSize > img.width ){
                resolve(img.src)
            } else {
            ...
            try {
                ...
                vu.sop.screenRecorder.recorder.onstop = e => {
                    ...
                    try {
                        ...
                        vu.sop.screenRecorder.completeBlob = new Blob(chunks, {type: videoType});
...
vu.sop.steps.document = async function() {
    let promise = new Promise(function (resolve, reject) {
        ...
        vu.sop.steps.takePictureDocument()
    });
    return promise;
}
...
vu.sop.steps.authFace = async function() {
    ...
    while (true) {
        ...
        try {
            ...
            if(vu.sop.recordProcess === true) {
                ...
                if(response === undefined || response.code != 2000) {
    ...
    if (vu.sop.disableBiometric) {
        ...
        if (response.code != 1907 &&
            response.code != 903) {
                await vu.error.showError(response.code);   
        }
    }
    return response
};
