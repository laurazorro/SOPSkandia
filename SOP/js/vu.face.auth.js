...
vu.face.auth.load = async function(basePath) {
    try {
        ...
        if (vu.face.useGestures == true) {
            ...
            let tfJsLoad =  vu.face.auth.loadJs(basePath + '/js/libs/tensorflowjs/3.9.0/tf.min.js');
            await tfJsLoad;
            let tfJsWasmLoad = vu.face.auth.loadJs(basePath + '/js/libs/tensorflowjs/3.9.0/tf-backend-wasm.min.js');
            await tfJsWasmLoad;
        } else {
